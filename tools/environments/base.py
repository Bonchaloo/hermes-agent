...
def _export_dump_excluding_session_vars(
    ...
    the whole group. *tmp_path* is normally the quoted, per-writer ``mktemp``
    result. A redirect on a pipeline segment would expand that variable inside
    the segment's subshell, potentially inconsistently with the parent that
    expands the follow-up ``mv``. Use ``>|`` because ``mktemp`` already created
    the target and a login profile may legitimately enable Bash ``noclobber``.
    """
...
class BaseEnvironment(ABC):
    ...
    def init_session(self):
        ...
        bootstrap = (
            ...
            f"  {_export_dump_excluding_session_vars(_snap_tmp, snapshot_excluded)} || exit 125\n"
